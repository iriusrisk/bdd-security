	...
	public static void main(String[] args) throws Throwable {
		...
		if (args[0].equals("tests")) {
			if (new File(report_dir).exists()) {
				Files.walk(Paths.get(report_dir)).sorted(Comparator.reverseOrder()).map(Path::toFile)
						.forEach(File::delete);
			}
			// Test execution starts here
			String[] args1 = { "--plugin", "pretty", "--glue", "net.continuumsecurity.steps", "classpath:features",
		...
		if (args[0].equals("reports")) {
			...
			net.masterthought.cucumber.Configuration configuration = new net.masterthought.cucumber.Configuration(
					new File(report_pretty_dir), projectName);
